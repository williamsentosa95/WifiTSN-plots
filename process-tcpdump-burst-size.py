...
def get_interarrival_time2(traces):
    inter = []
    for i in range(1, len(traces)):
        inter.append(traces[i][0] - traces[i-1][3])
...
plot_ecdf(ax1, uplink_burst_size, "uplink", "blue", 2)
plot_ecdf(ax1, downlink_burst_size, "downlink", "red", 2)


ax1.grid(zorder=-1)
ax1.yaxis.label.set_size(axis_label_font_size)
ax1.set(xlabel='Burst duration ($ms$)')
ax1.set(ylabel='CDF')
ax1.legend()
...
fig.tight_layout()
plt.savefig('web-burst-size.pdf', bbox_inches='tight')
